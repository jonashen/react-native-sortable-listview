...
class SortableListView extends React.Component {
  constructor(props, context) {
    ...
    this.state = {
      ...
      order: props.order || Object.keys(props.data) || [],
    };
    ...
    this.state.panResponder = PanResponder.create({
      ...
      onPanResponderRelease: () => {
        ...
        const fromIndex = this.state.order.indexOf(
          this.state.active.rowData.index
        );
        let toIndex =
  ...
  renderRow = (data, section, index, highlightfn, active) => {
    ...
    const hoveringIndex =
      this.state.order[this.state.hovering] || this.state.hovering;
    return (
      <Component
        {...this.props}
  ...
  componentDidUpdate(props) {
    if (props.order != this.state.order)
      this.setState({ order: props.order || Object.keys(props.data) || [] });
  }
